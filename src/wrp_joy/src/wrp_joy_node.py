...
class TeleopWR():
    ...
    def __init__(self):
        ...
        self.ats_ = message_filters.appoximateTimeSynchronizer([self.ultra0_,self.ultra1_,self.ultra2_,self.ultra3_], queue_size=10, slop = 0.1)
        self.ats_.registerCallback(self.ultra0_, self.ultra1_, self.ultra2_, self.ultra3_)
        ...
        #suscriber    
        self.cmd_adas_sub_ = rospy.Suscriber('/cmd_vel_adas', Twist, self.adas_CB)
        ...
        self.adas_trigger_pub_ = rospy.Publisher('cmd_vel_driver', Twist, queue_size=100)
    ...
    def joyCallback(self, joy):
            ...
            if (joy.buttons[4] != 1):
                ...
                if (joy.axes[self.dec_idx] > -1 ):
                    with lock:
                        ...
                        twist_data.linear.x = max(self.velo_x_ - min(self.safe_scale_ * self.l_scale_ * (joy.axes[self.acc_idx_] +1 ), self.acc_limit_), 0.0)                         
                elif (joy.axes[self.acc_idx_] > self.threshold_):
                    with lock:
                        #ROS_WARN("The acc velocity is:%f", velo_x_)
                        twist_data.linear.x = min(self.velo_x_ + min(self.safe_scale_ * self.l_scale_ * (joy.axes[self.acc_idx_] + 1), self.acc_limit_), self.velo_limit_)
                elif (joy.axes[self.acc_idx_] > -1 and joy.axes[self.acc_idx_] <= self.threshold_):
                    with lock:
                        #ROS_WARN("The maintain velocity is:%f", velo_x_)
                        twist_data.linear.x = min(self.velo_x_, self.velo_limit_)
                else:
                    with lock:
                ...
                if (joy.axes[self.dec_idx_] > -1):
                    with lock:
                        ...
                        twist_data.linear.x = min(self.velo_x_ + min(self.safe_scale_ * self.l_scale_ * (joy.axes[self.dec_idx_] + 1), self.acc_limit_), 0.0)
            
                elif (joy.axes[self.acc_idx_] > -1):
                    with lock:
                        #ROS_WARN("THHHHE acc velocity is:%f", velo_x_)
                        twist_data.linear.x = max(self.velo_x_ - min(self.safe_scale_ * self.l_scale_ * (joy.axes[self.acc_idx_] + 1), self.acc_limit_), -self.velo_limit_)
        ...
        twist_data.linear.x  = cmd_vel_lon_sum / max((len(self.twist_buffer_)), 1)
        twist_data.angular.z  = cmd_vel_lon_sum / max((len(self.twist_buffer_)), 1)
