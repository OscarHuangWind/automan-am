...
namespace human_robot_interaction {
    ...
    bool HRS::CalFinalVelocityCmd(geometry_msgs::Twist& vel_cmd_final)
    {
        ...
        if (flag)
        {
            ...
            ROS_WARN("Oscar::Driver is doing good.");
        }
        else
        {
			...
			if (vel_adas_.linear.z < 10)
			{
				//ROS_WARN("Oscar::Safe Stop.");
				weight_adas_cmd_lon_ = 1.0;
				weight_adas_cmd_rot_ = 0.0;
				weight_driver_cmd_lon_ = 1.0 - weight_adas_cmd_lon_;
				weight_driver_cmd_rot_ = 1.0 - weight_adas_cmd_rot_;
			...
			{
				//ROS_WARN("Oscar::Power Steering.");
				double lat_risk_human = vel_adas_.angular.x;
				double lat_risk_auto  = vel_adas_.angular.y;
				ROS_WARN("Oscar::The x:%f, y:%f, risk_human:%f, risk_auto:%f", vel_adas_.angular.x, vel_adas_.angular.y, lat_risk_human, lat_risk_auto);
				weight_adas_cmd_lon_ = lat_risk_human/(lat_risk_human + lat_risk_auto);
				weight_adas_cmd_rot_ = lat_risk_human/(lat_risk_human + lat_risk_auto);
				weight_driver_cmd_lon_ = 1.0 - weight_adas_cmd_lon_;
				weight_driver_cmd_rot_ = 1.0 - weight_adas_cmd_rot_;
			}	                    

			/* last version of shared control - lack of support
    ...
    void HRS::CalculateVelocity(geometry_msgs::Twist& vel_cmd)
    {
        ...
        vel_cmd.linear.x  = weight_adas_cmd_lon_ * std::max(0.0, vel_adas_.linear.x) + weight_driver_cmd_lon_ * vel_cmd.linear.x;
